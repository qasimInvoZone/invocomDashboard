        ...
        {user?.role === "SUPER_ADMIN" || user?.role === "ADMIN" ? (
          <ul>
            {chats?.length > 0 &&
              chats?.map((chat) => (
                <li
                  href="/apps/chat/messages"
                  className="active"
                  onClick={() => renderChat(chat._id)}
                >
                  <div className="chatList">
                    ...
                    <div className="desc">
                      {chat.unReadMessages > 0 && (
                        <div className="counter">{chat.unReadMessages}</div>
                      )}
                      <h5 style={{ fontWeight: "bold" }}>
                        {user?.role === "ADMIN" || user?.role === "SUPER_ADMIN"
                          ? chat?.client?.fullname
                          : chat?.superAdmin?.fullname}
                      </h5>
                      <p>
          ...
          "There is no chat assigned to you"
        )}
      </div>
    </>
  );
